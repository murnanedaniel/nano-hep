...
srun --jobid=${JOBID} --exact -N1 -n1 --gpus-per-task=1 \
  bash -c "cd ${REPO} && unset TMPDIR && WANDB__SERVICE_WAIT=300 ${PYTHON} -u -m nano_hep.train_hep --config configs/nano_hep_overnight_1M.yml" \
  2>&1 | tee runs/overnight_1M.log
